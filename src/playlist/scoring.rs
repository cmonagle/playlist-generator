...
impl PlaylistScoring {
    ...
    pub fn calculate_preference_score(song: &Song, config: &PlaylistConfig) -> f32 {
        ...
        if let Some(play_count) = song.play_count {
            if weights.discovery_mode {
                ...
                score += (base_discovery_score - (play_count as f32).min(base_discovery_score)) * weights.play_count_weight;
            } else {
    ...
    pub fn calculate_quality_score(songs: &[Song], metadata: &PlaylistMetadata, config: &PlaylistConfig) -> f32 {
        if songs.is_empty() {
            return 0.0;
        }
    ...
    pub fn calculate_genre_coherence_score(genre_distribution: &HashMap<String, usize>, total_songs: usize) -> f32 {
